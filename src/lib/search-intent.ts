...
export function detectSearchIntent(query: string, localResults: any[] = []): SearchIntent {
  ...
  const hasPartialMatch = localResults.some(res => {
    ...
    return titles.some(t => {
      ...
      return normT.includes(normalizedQuery) || 
             ...
             getSimilarity(normT, normalizedQuery) >= 0.75;
    });
  });
  ...
  // Default to PARTIAL_TITLE if there are some results, otherwise KEYWORD_EXPLORATION for external discovery
  return localResults.length > 0 ? 'PARTIAL_TITLE' : 'KEYWORD_EXPLORATION';
}
