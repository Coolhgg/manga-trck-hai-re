...
import { checkRateLimit, handleApiError, ApiError, ErrorCodes, sanitizeInput, sanitizeText, validateOrigin, USERNAME_REGEX } from "@/lib/api-utils"
import { z } from "zod"
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!checkRateLimit(`users-me:${ip}`, 60, 60000)) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const ip = request.headers.get("x-forwarded-for") || request.headers.get("x-real-ip") || "unknown"
    if (!checkRateLimit(`users-me-update:${ip}`, 20, 60000)) {
      throw new ApiError('Too many requests. Please wait a moment.', 429, ErrorCodes.RATE_LIMITED)
    }
    ...
    const { username, bio, avatar_url, notification_settings, privacy_settings, safe_browsing_mode, safe_browsing_indicator } = validatedBody.data
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const ip = request.headers.get("x-forwarded-for") || request.headers.get("x-real-ip") || "unknown"
    if (!checkRateLimit(`users-me-delete:${ip}`, 5, 3600000)) {
    ...
    // Delete user from database (cascading delete should handle related entries if configured)
    await withRetry(
      () => prisma.user.delete({
        where: { id: user.id }
      }),
    ...
    if (deleteError) {
      ...
      // We continue since the DB record is gone, but this is a partial failure
    }
