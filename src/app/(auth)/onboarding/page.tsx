...
import { CheckCircle2, CloudDownload, Search, Bell, ArrowRight, Loader2, Plus, X, Star, Users } from "lucide-react"
  ...
  const handleImport = async (platform: string) => {
    ...
    setTimeout(() => {
      ...
      toast.success(`Import from ${platform} completed!`, {
        description: "Your library has been synced.",
      })
    }, 3000)
  }
            ...
            <div className="space-y-6">
              {!importing ? (
                <div className="grid grid-cols-1 gap-4">
                  <Button
                    variant="outline"
                    className="h-16 rounded-2xl border-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 justify-start px-6 gap-4"
                ...
                <div className="py-12 space-y-6 text-center">
                  <Loader2 className="size-12 animate-spin mx-auto text-zinc-900 dark:text-zinc-50" />
                  <div className="space-y-2">
                    <p className="font-bold text-lg">Importing from {importPlatform}...</p>
                    <Progress value={progress} className="h-2 rounded-full" />
                    <p className="text-xs text-zinc-500 font-medium">{progress}% Complete</p>
                  </div>
                </div>
              )}
            </div>
